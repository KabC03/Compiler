...
def write_to_c_file(cFileName: str, hFileName: str, verboseOutput: bool) -> int:
    ...
    try:
        ...
        with open(str(cFileName), 'w') as cFile:
            ...
            cFile.write("std::vector<std::string> global_reservedTokens = {\n")
...
def write_to_h_file(hFileName: str, verboseOutput: bool) -> int:
    ...
    try:
        with open(str(hFileName), 'w') as hFile:
            hFile.write("#ifndef TOKENS_HPP\n")
            hFile.write("#define TOKENS_HPP\n\n")
            hFile.write("#include <string>\n")
            hFile.write("#include <vector>\n\n")
            hFile.write("extern std::vector<std::string> global_reservedTokens;\n")
...
def check_cmd_args(argv: List[str]) -> Tuple[int, bool, str, str]:
    ...
    for item in argv:
        ...
        if item in __file__:
            pass
        elif item == "-v" and not verboseOutput:
            verboseOutput = True
        elif item.endswith((".cpp", ".c++", ".cc")) and not cFileName:
            cFileName = item
        elif item.endswith((".hpp", ".h++", ".hh")) and not hFileName:
            hFileName = item
        else:
